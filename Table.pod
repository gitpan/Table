...
C<$frame> is the pathname of a window which must already exist.
Blt_table will arrange its slave windows inside of C<$table>.  Slave
...
the rows of the table specified in argument will have the same heigth than 
the rows of the table. 

=item -col

the columns of the table specified in argument will have the same width than 
the columns of the table.

=item -xscrollincrement

=item -yscrollincrement
...
the width slave window is expanded.  If is y, the height is
...
=head1 Columns and rows configuration
...
=item $table->get($row, $column);

return the slave in the cell ($row, $column).

=item $table->under($x, $y);

return the slave located at $x, $y

=item $table->dimension;

return the number of rows and columns of the table.

=item $table->tablex($x);

Given a screen x-coordinate C<$x> this command returns the
table x-coordinate that is displayed at that location.

=item $table->tabley($y);

Given a screen y-coordinate C<$y> this command returns the
table y-coordinate that is displayed at that location.
...
=head1 Bounding list format

Constraints for various options (-reqheight, -reqwidth, -height,
and -width) are specified by supplying a bounding list of values.
...
Set the size of the window or partition at C<$x>.  The
size  of  the  partition  or window cannot grow or shrink.

=item [ $min,  $max ]

Bound the size of the window or partition  between C<$min>
and C<$max>.

=item [ $min, $max, $nom ]

Bound the size of the window or partition  between
C<$min>  and C<$max>.  In addition, set the normal size to
C<$nom>.


The maximum bound max can also be specified as C<"Inf"> to
